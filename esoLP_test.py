...
parser.add_option('--aperMulti',
                  dest='aperMulti',action='store',default=2.5,#default=1.3
                  type='float',
...
parser.add_option('--lineAperMulti',
                  dest='lineAperMulti',action='store',default=2.5,#default=1.2
                  type='float',
                  help='The round aperture radius (times the FWHM) to be used in the photometry. DEFAULT=%default')
...
parser.add_option('--PS1',action='store_true',dest='PS1',default=False,
...
#    if target in fullList[i]:
#        s=fullList[i].split()
#        if s[3] in ['z_G0304','r_G0303','g_G0301']:
#            files.append(s[0].replace('.gz',''))
#            filts.append(s[3])
...
if 1:
    for i in range(len(files)):
        ...
        elif filts[i]=='r': nominalZPT=28.34
        ...
        catName=path+files[i].split('.')[0]+'.cat'
        print
        if opt.runSex: scamp.runSex('%s.sex'%(target), path+files[i],options={'CATALOG_NAME':catName},verbose=True)
        catalog=scamp.getCatalog(catName,paramFile='def.param')
        trimmedCatalog=catalog#catalogTrim(catalog, opt.moffatsnr)

        with pyf.open(path+files[i]) as han:
            data=han[0].data
            header=han[0].header
        ...
        if not opt.loadPSF:
            ...
            goodPSF.psfStore(path+files[i].split('.')[0]+'.psf.fits')


            with open(path+files[i].split('.')[0]+'.psfStars.pickle','w+') as psfHan:
            ...
            goodPSF=psf.modelPSF(restore=path+files[i].split('.')[0]+'.psf.fits')
            goodPSF.fitted=False
            fwhm=goodPSF.FWHM(fromMoffatProfile=True)
            goodPSF.fitted=True

            with open(path+files[i].split('.')[0]+'.psfStars.pickle') as inHan:
                [goodFits,sourceAperCorrs,prevRoundAper]=pickle.load(inHan)
            prevRoundAperCorrError=num.nanstd(sourceAperCorrs)*len(sourceAperCorrs)**-0.5

            roundAperCorrError=-2.5*num.log10(1.-(10.0**(prevRoundAperCorrError/2.5)-1)*10.0**((goodPSF.roundAperCorr(opt.aperMulti*fwhm)-goodPSF.roundAperCorr(prevRoundAper))/2.5))
            lineAperCorr=goodPSF.lineAperCorr(opt.lineAperMulti*fwhm)
            roundAperCorr=goodPSF.roundAperCorr(opt.aperMulti*fwhm)
            lineAperCorrError=-2.5*num.log10(1.-(10.0**(roundAperCorrError/2.5)-1)*10.0**((lineAperCorr-roundAperCorr)/2.5))

            print '\nProfile has FWHM of %s pix or %s arcsec.'%(fwhm,fwhm*0.0728)
            print "Using round aperture of %s pixel radius with aperture correction of %s mags."%(fwhm*opt.aperMulti,roundAperCorr)
            print 'Using line aperture correction of %s magnitudes and uncertainty %s.\n'%(lineAperCorr,lineAperCorrError)
            print 'The uncertainty on the round aperture corrections is %s mags.\n'%(roundAperCorrError)
        ...
        if opt.PS1:
            ...
            ps1=ps1_sdss.sdssPS1(catPath='/Users/fraserw/git/ColOSSOS/catalogs/PSCatalogsFromCharles/')
        ...
        if opt.doPSFPhotometry:
            (fit_x,fit_y,fit_amp,chi)=goodPSF.fitWithModelPSF(cat[0],cat[1],data,fitWidth=int(2*fwhm),useLinePSF=True,verbose=False,bg=phot.bg)
            print 'The corrected model flux is %.2f using correction %.4f'%(goodPSF.fitFlux,goodPSF.fitFluxCorr)
        ...
        try:
            ...
            if len(sdss)<>0:
                ...
                for j in range(len(sdss)):
                    (sdsscat,flag)=getClosestSource(catalog,sdss[j][0],sdss[j][1])
                    ...
                    if pix_delt<15 and flag==0:
                        ...
                        elif filts[i] == 'g_G0301':
                            errm = (sdss[j][10] ** 2 + phot.dmagnitude ** 2) ** 0.5
                        elif filts[i] == 'z_G0304':
                ...
                (sdssZmagsTrim,dzpTrim,edzpTrim,sdssgsTrim,sdssrsTrim,sdsszsTrim)=(sdssZmags,dzp,edzp,sdssgs,sdssrs,sdsszs)#sigTrim(sdssZmags,dzp,edzp,sdssgs,sdssrs,sdsszs)
                ...
                pyl.xlabel('Observed Magnitude '+filts[i].replace('_','.'))
                ...
                acceptedStars[0]-=mm
                with open('%s_sdssStars.pickle'%(path+files[i].split('.')[0]), 'w+') as sdssHan:
                    pickle.dump(acceptedStars,sdssHan)
            else:
                entry.append(nominalZPT)
                entry.append(0.04)

            print '\n\n'
            #print entry

            #correct the read in entry with the new zeropoint
            if opt.redoZeropoint:
                print 'here'
                redoEntry[-7]+=entry[-2]-redoEntry[-2]
                print 'here'
                redoEntry[-2]=entry[-2]
                print 'here'
                redoEntry[-1]=entry[-1]
