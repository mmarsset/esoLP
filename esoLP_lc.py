...
path='/Users/mmarsset/data/ESOLP/20150117/'+target+'/REDUCED/'
...
if 1:
    for i in range(len(files)):
        ...
        fname = files[i].split('.')[0]+'.'+files[i].split('.')[1]+'.'+files[i].split('.')[2]
        ...
        if not opt.loadPSF:
            ...
            goodPSF=psf.modelPSF(restore=path+fname+'.psf.fits')
            goodPSF.fitted=False
            fwhm=goodPSF.FWHM(fromMoffatProfile=True)
            goodPSF.fitted=True

            with open(path+fname+'.psfStars.pickle') as inHan:
        ...
        try:
            ...
            if len(sdss)<>0:
                ...
                acceptedStars[0]-=mm
                with open('%s_sdssStars.pickle'%(path+fname), 'w+') as sdssHan:
